...
export function renderBookPreviewDocument(
	...
	return `<!doctype html><html><head><meta charset="utf-8"><title>${title}</title><style>${previewCss(design, readerScale)}</style><style id="book-designer-preview-layout"></style></head><body><article class="book">${book.sections
		.map((section, index) => `<section class="chapter chapter-${escapeAttribute(design.chapterStyleId)}" id="${escapeAttribute(section.id)}" data-section-id="${escapeAttribute(section.id)}"><header><p class="chapter-number">${chapterLabel(index + 1, design.chapterStyleId)}</p><h1>${escapeHtml(section.title)}</h1></header>${section.blocks.map(renderBlock).join('')}</section>`)
